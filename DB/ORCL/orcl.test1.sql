...
CREATE SEQUENCE AI_SEQ_JOURTNALPOSTS START WITH 1 MINVALUE 1 MAXVALUE 9999999999999999999999999999;
...
CREATE OR REPLACE TRIGGER JOURTNALPOSTS_AUTO_INCREMENT 
BEFORE INSERT ON JOURTNALPOSTS 
FOR EACH ROW

BEGIN
  SELECT AI_SEQ_JOURTNALPOSTS.NEXTVAL INTO   :NEW.ID FROM   dual;
END;
...
CREATE OR REPLACE TRIGGER BIN_BUP_JOURTNALPOSTS_TR
BEFORE INSERT OR UPDATE ON JOURTNALPOSTS FOR EACH ROW

BEGIN
  IF INSERTING THEN
  	SELECT AI_SEQ_JOURTNALPOSTS.NEXTVAL INTO   :NEW.ID FROM   dual;
    :NEW.CODE := TO_CHAR(SYSDATE,'YYYY') + AI_SEQ_JOURTNALPOSTS.CURRVAL;
