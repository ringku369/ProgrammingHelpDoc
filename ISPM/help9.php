...
update accjaopenbalances set uopnbdebit = 0, uopnbcredit = 0, debit = 0, credit = 0, clbdebit = 0, clbcredit = 0 where warehouse_id = @whid;
...
update accjaopenbalances set uopnbdebit = ((t2.opnbdebit) + (t2.debit - t2.credit)), uopnbcredit = 0
from accjaopenbalances join (
  select * from cte2
) as t2 on t2.id = accjaopenbalances.id where 
...
update accjaopenbalances set uopnbdebit = 0, 
uopnbcredit = ((t2.opnbcredit) + (t2.credit - t2.debit))
from accjaopenbalances join (
  select * from cte3
) as t2 on t2.id = accjaopenbalances.id where 
...
with cte2 as (
  select t3.id as id, t3.accjournalaccount_id as aid, 
  t3.uopnbdebit, t3.uopnbcredit,
  t3.debit, t3.credit,
  t3.clbdebit, t3.clbcredit
  from accjaopenbalances t3 where t3.accstatus = 1 and t3.warehouse_id = @whid
)
update accjaopenbalances set clbdebit = ((t2.uopnbdebit) + (t2.debit - t2.credit)), clbcredit = 0
from accjaopenbalances join (
  select * from cte2
) as t2 on t2.id = accjaopenbalances.id where 
...
update accjaopenbalances set clbdebit = 0, 
clbcredit = ((t2.uopnbcredit) + (t2.credit - t2.debit))
from accjaopenbalances join (
  select * from cte3
) as t2 on t2.id = accjaopenbalances.id where 
...
--Exec TB_Stap_1_11 @wid = 2, @fdate = '2021-04-03', @tdate = '2021-04-03';
